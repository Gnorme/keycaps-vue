<template>
    <div>
      <div id="customKey" class="grid-container">
          <key-customizer :main-style="mainStyle" :img='surfaces[currentView].img.value' :text='surfaces[currentView].text.value' :current-view='currentView' :transform-text='transformText' :transform-img='transformImg' :color='selectedColor.color' :background-color='shadeBlend(-0.2,selectedColor.color)' :stroke-color='shadeBlend(0.065,selectedColor.color)' :selected-key='selectedKey' :set-color='setColor' :colors='colors' :on-change='previewImg' :filename='surfaces[currentView].img.filename' :change-view='changeView' :view-options='viewOptions'></key-customizer>   
            <key-selector :keys='keys' :active='active' :add-to-cart="addToCart"></key-selector>
      </div>
      <keycap-product-info></keycap-product-info>
    </div>
</template>

<script>
    import Vue from 'vue/dist/vue.js'
    import KeyCustomizer from './KeyCustomizer.vue'
    import KeySelector from './KeySelector.vue'
    import KeycapProductInfo from './KeycapProductInfo'
    ...
    export default {
        name:'CustomKey',
        mixins:[DetectMobile],
        components:{KeyCustomizer, KeySelector, KeycapProductInfo},
        data: function() {
            return {
                currentView:'topView',
    ...
    methods: {
        ...
        Vue.nextTick(function () {
            ...
            interact.interact('.moveableText').options.drag.snap.targets = self.calcSnapTargets(surface,textWidth,textHeight)
            ...
            interact('.moveableImg').options.drag.snap.targets = self.calcSnapTargets(surface,imgWidth,imgWidth)
        ...
        interact('.moveableText').options.drag.snap.targets = this.calcSnapTargets(surface, newWidth, newHeight)
        },
        ...
        if (file) {
            ...
            interact('.moveableImg').options.drag.snap.targets = this.calcSnapTargets(surface,width,width)
        } else {
            target.value = "";
        }       
        },
        calcSnapTargets: function() {
...
function addInteractive() {
    ...
    interact('.moveableImg')
        ...
        ck.surfaces[ck.currentView].img.width = target.clientWidth
        //ugly hack, can't figure out how to shrink div to image size properly. Tried css tricks eg inline-block, float, table, 
        cl.surfaces[ck.currentView].img.height = target.clientHeight - 5

        interact('.moveableImg').options.drag.snap.targets = ck.calcSnapTargets(surface, event.rect.width, event.rect.height)    
        //placeImage()
        });
}   
function dragMoveListener (event) {
  ...
  if (target.className == "moveableImg"){
    var node = ck.surfaces[ck.currentView].img
  } else if (target.className == "moveableText") {
    var node = ck.surfaces[ck.currentView].text
  }
...
document.body.addEventListener('click', function(e) {
    ...
    if (target.classList.contains('key-face')) {
      ck.changeKey(target.getAttribute('name'))
    }     
    }
    e.stopPropagation()
}); 
